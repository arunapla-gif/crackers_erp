...
export default function PendingOrders() {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const data = await erpApi.getPendingSalesOrders();
      setOrders(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-6 pt-4">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={fetchOrders} className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-4 py-2 rounded-xl text-sm transition-colors shadow-sm">
          🔄 Refresh
        </button>
      </div>
            ...
            return (
              <div key={order.id} className="bg-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.04)] border border-slate-200 overflow-hidden group hover:border-active/30 transition-colors">
                ...
                <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex flex-col md:flex-row justify-between md:items-center gap-4">
                  <div>
                    <span className="inline-block px-3 py-1 bg-amber-100 text-amber-700 rounded-lg text-xs font-black tracking-widest mb-2">PENDING APPROVAL</span>
